...
执行顺序：
1. **先读锁定范围文档**：`当前锁定范围与禁止主动重构清单.md` + `当前阶段目标与范围.md` 中的"明确不在范围内"段落
2. **将禁止事项列为否决条件**：任何匹配禁止事项的请求 → 自动否决
3. **否决后不进入评估**：被否决的请求不需要再分析利弊、可行性或技术方案
4. **只有未被否决的请求**才进入下一步：评估优先级、资源、影响范围

典型禁止事项（应出现在锁定范围文档中）：
- 不在当前阶段范围内的功能扩展
- 已锁定的接口/模型变更
- 未经 cross-area 评估的共享模型修改
- 单人决策的跨单元契约变更

**PM 特别约束**：PM 做范围决策时，必须在输出中显式列出：
- 读了哪些否决条件文档
- 本次请求中有哪些被否决（如有）
- 否决依据是哪个文档的哪条规则

如果一个请求看起来"很合理"但触及了当前范围的禁止事项，必须先更新范围文档（经正常流程），再批准请求。不得跳过范围文档直接批准。

### 禁止事项粒度原则

**禁止事项必须使用具体名称，禁止使用分类概括**。模型会对"看起来合理的功能"产生合理化偏见，绕过模糊的禁止条款。
...
**规则**：每一条禁止事项必须至少包含一个具体的字段名、接口路径、文件名或包名。如果某条禁止事项无法举出具体名称，说明它还不够清晰，需要进一步细化。

**应用于锁定范围文档**：`当前锁定范围与禁止主动重构清单.md` 和 `当前阶段目标与范围.md` 中的"明确不在范围内"段落，必须遵循此粒度原则。bootstrap 和 patch 模式下创建的模板应包含具体名称的占位符，不应预填分类概括。
...
当任务满足以下全部条件时，跳过项目状态层和完整路由表，直接进入代码约束层的最小加载：

1. **单文件**：修改范围只涉及一个文件
2. **非结构性**：不改变类/模块/接口的签名、职责或依赖关系
3. **非首次**：不是创建新文件、新类或新公共 API
4. **非安全关键**：不涉及认证、授权、加密、资金计算、数据删除

典型适用场景：typo 修复、变量重命名、单行 bug 修复、注释修正、格式调整。

快速通道下只加载 `references/warning-signs.md`（[核心] 规则部分），跳过其他参考文件。如果执行中发现实际范围超出以上条件，回退到完整路由流程。
...
0. **Always load `references/overrides.md` [核心] first** — overrides define which legacy rules are downgraded or disabled for modern projects. An override always wins over the rule it references.
1. **Then, scan `[核心]` rules** in the target file(s) — these cover ~10-30% of rules, are the minimal safe working surface, and always apply.
2. **Read `[扩展]` rules** only when:
...
| quality check / informal desk check / inspection | references/process.md |
| code review / PR review / merge readiness check | references/process.md, references/git-workflow.md, references/project-state.md |

`references/overrides.md` and `references/warning-signs.md` apply to all coding activities — load them first, then the routed files.
